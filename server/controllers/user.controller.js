...
const registerProfile = async (profile) => {
  ...
  const user = await registerUser(fullname, password, email, true);
  ...
  try {
    ...
    if (currUser)
      return res.status(409).json({
        success: false,
        msg: "An account with this email already exists! Try a different better one...",
      });
...
const login = async (req, res, next) => {
  try {
    ...
    const isPasswordValid = utils.checkPassword(
      req.body.password,
      user.hash,
      user.salt
    );
