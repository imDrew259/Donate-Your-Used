...
export default function Globe() {
      ...
      if (response.status == 200) {
        ...
        for (var i = 0; i < response.data.clothes.length; i++) {
          const obj = {
            ...
            coordinates: [
              response.data.clothes[i].location.latitude,
              response.data.clothes[i].location.longitude,
            ],
  ...
  async function onAddFormSubmit() {
    if (user) {
      ...
      data.append("upload_preset", "doocf3du");
      console.log(data);
      fetch(process.env.cloudinaryAPI, {
        method: "post",
        body: data,
      })
        .then((res) => res.json())
        .then(async (data) => {
          ...
          const response = await axios.post(
            "http://localhost:8000/api/clothes",
            addClothData,
            { headers: headers }
          );
  ...
  return (
    <>
      ...
      <div className="modal" style={{ display: showAddFestival }}>
        <div className="form">
          ...
          <div className="buttons">
            <div className="explore-button extra" onClick={getLocation}>
              Get Coordinates
            </div>
            <div className="explore-button" onClick={onAddFormSubmit}>
              Add
            </div>
            <div
      ...
      {currentFestival ? (
        ...
        <div
          className="explore-button corner"
          onClick={() => {
